...
def convert_coco_json(json_dir='../coco/annotations/', use_segments=False):
    out_dir = os.path.join(json_dir, "../yolo")
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            for ann in anns:
                ...
                box = [cls] + box.tolist()
                if ann['bbox'] not in orig_bbox:
                    orig_bbox.append(ann['bbox'])
                    bboxes.append(box)
                # Segments
                if use_segments:
...
if __name__ == '__main__':
    ...
    if source == 'COCO':
        convert_coco_json('outputs/Cityscapes_coco/annotations',  # directory with *.json
                          use_segments=False)
